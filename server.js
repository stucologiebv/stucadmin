...
app.get('/api/health', (req, res) => {
    ...
    res.json({ 
        ...
        data: {
            materialen: materialenDB?.length || 0,
...
app.post('/api/medewerker/login', (req, res) => {
    ...
    for (const m of companyMedewerkers) {
        ...
        if (m.pincode && m.pincode.includes(':')) {
            // Gehashte pincode - gebruik verifyPassword
            if (verifyPassword(pincode, m.pincode)) {
                medewerker = m;
                break;
            }
        } else {
            // Plaintext pincode (legacy) - directe vergelijking
            if (m.pincode === pincode) {
                medewerker = m;
                needsMigration = true;
                break;
            }
...
app.get('/api/medewerker/uren', requireMedewerkerAuth, (req, res) => {
    console.log(`ðŸ“Š Uren opvragen voor: ${req.medewerker} (${req.medewerkerId}), totaal uren in db: ${uren.length}`);
    const mijnUren = uren.filter(u => u.medewerkerId === req.medewerkerId);
    console.log(`ðŸ“Š Gevonden: ${mijnUren.length} uren`);
    res.json(mijnUren);
});
...
app.post('/api/offerteaanvragen', (req, res) => {
    ...
    const webhookSecret = process.env.WEBHOOK_SECRET || 'stucadmin-webhook-2024';
    ...
    console.log(`ðŸ“© Nieuwe offerteaanvraag: ${newAanvraag.naam} (${newAanvraag.type})`);
